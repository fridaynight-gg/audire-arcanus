...
const SOCKET_URL = 'http://localhost:5551';
...
let activeAudioSources = []; // Track active BufferSourceNode instances
// DOM Elements
...
function initializeAudioContext() {
    ...
    try {
        ...
        if (audioContext.sampleRate !== SAMPLE_RATE) {
            console.warn(`[AUDIO WARNING] Sample rate mismatch! Expected ${SAMPLE_RATE}Hz but got ${audioContext.sampleRate}Hz. Audio will be resampled and may sound distorted.`);
        }
...
function handleAudioData(data) {
    ...
    if (audioPacketCount <= 5) {
        console.log(`[AUDIO] Packet #${audioPacketCount}: ${byteCount} bytes, type: ${data.constructor.name}`);
    }
    try {
        ...
        const audioBuffer = audioContext.createBuffer(CHANNELS, samplesPerChannel, SAMPLE_RATE);
        // Log buffer creation for first few packets
        if (audioPacketCount <= 3) {
            console.log(`[AUDIO] Created buffer: ${samplesPerChannel} samples/channel, ${audioBuffer.duration.toFixed(3)}s duration, ${CHANNELS} channels, ${audioBuffer.sampleRate}Hz`);
        }
...
function saveSession() {
    if (currentLobby && username && currentJoinCode) {
        localStorage.setItem('audire_session', JSON.stringify({
            username,
...
async function joinLobbyWithCode() {
    ...
    socket.emit('join-lobby', {
        ...
        if (response && response.success) {
            ...
            updateConnectionStatus('disconnected');
            showError('Failed to join lobby: ' + (response?.error || 'Unknown error'));
...
function updateListenersList(listeners) {
    ...
    const listHTML = listeners
        .map((l) => `<div style="padding: 0.25rem 0; color: #000;">${l.username}</div>`)
        .join('');
...
async function refreshCurrentLobbyInfo() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const listenersResponse = await fetch(`${API_BASE}/lobby/${currentLobby.id}/listeners`);
...
function setupAudioControls() {
    ...
    if (outputDeviceSelect) {
        outputDeviceSelect.addEventListener('change', async (e) => {
            ...
            console.log('Output device changed to:', currentOutputDeviceId || 'Default');
